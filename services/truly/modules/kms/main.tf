...
#############################################################
locals {
  full_key_name           = format("%s-%s", data.aws_region.region.name, var.service) # sets key name for primary KMS Key
  full_key_name_secondary = format("%s-%s", data.aws_region.region_secondary.name, var.service) # sets key name for secondary KMS Key if multi-region key is created
}
